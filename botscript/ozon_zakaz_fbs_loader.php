...
if ($mode ==1){
    ...
    echo "Load FBS orders from ".$df." to ". $de;
    
    $query = "DELETE from workdata WHERE (cabinet='".$cabinet."' and data_type='fbs_last_refresh')";
...
function create_report($start_date_s, $end_date_s){
    ...
    if ($status==200){                                 
         ...
         $query = "DELETE from workdata  WHERE (cabinet='".$cabinet."' and data_type='fbs_report')";
         $a = mysqli_query($link,$query);
        
         $query = "INSERT INTO  workdata (cabinet, data_type, data_values) VALUES ('".$cabinet."', 'fbs_report', '".$report_id."')";
         $a = mysqli_query($link,$query);
...
function get_report($s_date, $e_date){
    ...
    global $cabinet, $link, $clientID, $apiKey;
        ...
        $query = "SELECT * FROM workdata WHERE (cabinet='".$cabinet."' and data_type='fbs_last_refresh')"; 
        ...
        if(!$date_refresh || $date_refresh!=$date_now ) {                                                                //если обновления не было
             ...
             $query = "SELECT * FROM workdata WHERE (cabinet='".$cabinet."' and data_type='fbs_report')";                   // получаем ID отчета
             $request = mysqli_query($link,$query);
             $arr = mysqli_fetch_assoc($request);
            ...
            if ($status==200){ 
                ...
                if($status == "success" && $file){
                    ...
                    // удаляем старые данные
                     ...
                     $query = "DELETE from zakaz_fbs  WHERE (cabinet='".$cabinet."'  and c2 >='".$s_date."'  and c2 <='".$e_date."')";
                     $a = mysqli_query($link,$query);
                        ...
                        for($i=1; $i<count($csv_data); $i++){ 
                           ...
                           if($csv_data[$i][0]) {$c0 = $csv_data[$i][0];} else {$c0=0;}
                            ...
                            if ($c0){
                                 ...
                                 $query = "INSERT INTO zakaz_fbs (cabinet, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13, c14, c15, c16, c17, c18, c19, c20, c21, c22, c23, c24)  VALUES ('".$cabinet."', '".$c0."', '".$c1."', '".$c2."', '".$c3."', '".$c4."', '".$c5."', '".$c6."', '".$c7."', '".$c8."', '".$c9."', '".$c10."', '".$c11."', '".$c12."', '".$c13."', '".$c14."', '".$c15."', '".$c16."', '".$c17."', '".$c18."', '".$c19."', '".$c20."', '".$c21."', '".$c22."', '".$c23."', '".$c24."')"; 
             ...
             $query = "INSERT INTO workdata (cabinet, data_type, data_values)  VALUES ('".$cabinet."','fbs_last_refresh','".$date_now."')";
             $a = mysqli_query($link,$query);  
